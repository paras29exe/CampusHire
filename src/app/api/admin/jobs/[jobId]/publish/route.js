...
export const PUT = withDB(async (req, { params }) => {
    ...
    try {
        ...
        if(job.eligibility_criteria.batches == [] || job.eligibility_criteria.courses == {}){
            return NextResponse.json({
                message: "Job cannot be published without eligibility criteria",
            }, { status: 400 });
        }
        ...
        if (!job.job_roles || job.job_roles.length === 0) {
            return NextResponse.json({
                message: "Job must have at least one job role to be published",
            }, { status: 400 });
        }

        if(job.links && job.links.company_link && !job.links.college_link){
            return NextResponse.json({
                message: "Job can not be published without company and college apply links",
            }, { status: 400 });
        }
        ...
        if (job.last_date_to_apply && new Date(job.last_date_to_apply) <= new Date()) {
            return NextResponse.json({
                message: "Job cannot be published without a last date or a past last date to apply",
            }, { status: 400 });
        }
        ...
        await Assignment.updateMany(
            { company: jobId },
            { $set: { status: 'in-progress' } } // Update all assignments related to this job to 'in-progress'
        );
        await job.save();
        ...
        return NextResponse.json({
            message: err.message || "Unexpected message occurred",
            error: "An message occurred while publishing the job",
        }, { status: 500 });
    }
});