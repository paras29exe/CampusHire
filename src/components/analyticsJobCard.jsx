...
export default function JobAnalyticsPage() {
    ...
    const handleViewAnalytics = (jobId, companyName) => {
        window.location.href = `/analytics/jobs/${jobId}?company=${companyName.toLowerCase().replace(/\s+/g, "-")}`
    }

    const handleViewDescription = (jobId, companyName) => {
        window.location.href = `/jobs/${jobId}?company=${companyName.toLowerCase().replace(/\s+/g, "-")}`
    }
                        ...
                        <Card key={job._id} className="hover:shadow-lg transition-shadow duration-300 border-0 shadow-md">
                            ...
                            <CardContent className="space-y-4">
                                ...
                                <div className="space-y-2 pt-2">
                                    <Button
                                        onClick={() => handleViewAnalytics(job._id, job.company.name)}
                                    ...
                                    <Button
                                        onClick={() => handleViewDescription(job._id, job.company.name)}
                                        variant="outline"
