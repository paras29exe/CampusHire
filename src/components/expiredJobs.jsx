  ...
  return (
    <Card className="w-full max-w-md relative overflow-hidden opacity-75 hover:opacity-90 transition-opacity">
      ...
      <CardHeader className="pb-3 flex flex-row justify-between">
        <div className="flex flex-col gap-y-1.5 max-w-1/2">
          <CardTitle className=" text-gray-700 line-clamp-2">{jobData.company.name}</CardTitle>
          <p className="text-sm text-blue-600 line-clamp-1">
            <a href={jobData.company.website} target="_blank" rel="noopener noreferrer">Visit Website</a>
          </p>
        </div>
      ...
      <CardContent className="space-y-4 mt-auto">
        ...
        <Button
          variant="outline"
          onClick={() => router.push(`/job-description?jobId=${jobData._id}`)}
