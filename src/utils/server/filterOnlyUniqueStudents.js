...
export const filterOnlyUniqueStudents = withDB(async (formattedData) => {
        ...
        const rollnos = formattedData.map(s => s.roll_number);
        ...
        const existingStudents = await Student.find({
            $or: [
                { roll_number: { $in: rollnos } },
                { email: { $in: emails } },
                { college_email: { $in: collegeEmails } },
                { phone: { $in: phones } }
            ]
        }, "roll_number email college_email phone");

        // Build sets for fast lookup
        const rollnoSet = new Set(existingStudents.map(s => s.roll_number));
        ...
        const newStudents = formattedData.filter(student =>
            !rollnoSet.has(student.roll_number) &&
